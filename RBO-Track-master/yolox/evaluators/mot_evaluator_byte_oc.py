...
class MOTEvaluator:
    ...
    def evaluator_byte_oc_track(
        ...
        ori_thresh = 0.65
        self.args.init_thresh = ori_thresh
...
#                     self.args.init_thresh = 0.65
#                 elif video_name == 'MOT17-06-FRCNN':
#                     self.args.init_thresh = 0.65
#                 elif video_name == 'MOT17-12-FRCNN':
#                     self.args.init_thresh = 0.7
#                 elif video_name == 'MOT17-14-FRCNN':
#                     self.args.init_thresh = 0.67
#                 else:
#                     self.args.init_thresh = ori_thresh

#                 if video_name == 'MOT20-06' or video_name == 'MOT20-08':
#                     self.args.init_thresh = 0.3
#                 else:
#                     self.args.init_thresh = ori_thresh
                ...
                if frame_id == 1:
                    tracker = Byte_oc_Track(args, init_thresh=self.args.init_thresh, determined_thresh=self.args.determined_thresh,
                ...
                if os.path.exists(ckt_file):
                    ...
                    # condition = outputs[0][:, 4] * outputs[0][:, 5] > self.args.determined_thresh
                    # determined_outputs = outputs[0][condition]
                    # rest_outputs = outputs[0][~condition]
                    # condition1 = outputs[0][:, 4] * outputs[0][:, 5] > self.args.init_thresh
                    # init_outputs = rest_outputs[condition1]
                    # lenoft += len(outputs[0])
                    # print("len of outputs:", lenoft)
                    if outputs[0] == None:
            ...
            # run tracking
            online_targets = tracker.update_b(outputs[0], info_imgs, self.img_size,
                                            warp_matrix=warp_matrix)  # [hgx0411] id_feature
            # print("len of online targets:", len(online_targets))
            online_tlwhs = []
            online_ids = []
